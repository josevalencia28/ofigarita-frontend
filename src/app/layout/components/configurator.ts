...
export class Configurator {
    ...
    ngOnInit() {
        ...
        const primaryPalette = this.primaryColors().find(p => p.name === config.primary);

        $t()
            .preset(preset)
            .preset(this.getPresetExt()) // 🔴 AQUÍ ENTRA OFI
  ...
  50:  '#fff5f8',   // Rosa muy suave (fondos)
  100: '#ffe1ec',   // Rosa pastel
  200: '#ffb8d2',   // Rosa claro
  300: '#ff8bb6',   // Rosa medio
  400: '#ff5c99',   // Rosa vibrante
  500: '#ff2f7d',   // Rosa principal (logo)
  600: '#e61f6c',   // Rosa profundo
  700: '#b51652',   // Rosa oscuro
  800: '#7a0f38',   // Vino suave
  900: '#3d081c',   // Casi marrón
  950: '#1f040e'    // Oscuro extremo
}
    ...
    primaryColors = computed<SurfacesType[]>(() => {
        ...
        const customPalettes: Record<string, SurfacesType['palette']> = {
    ofi: {
        0: '#ffffff',
        50: '#fff5f8',
        100: '#ffe1ec',
        ...
        const colors = ['ofi','emerald', 'green', 'lime', 'orange', 'amber', 'yellow', 'teal', 'cyan', 'sky', 'blue', 'indigo', 'violet', 'purple', 'fuchsia', 'pink', 'rose'];
        const palettes: SurfacesType[] = [{ name: 'noir', palette: {} }];

colors.forEach((color) => {
    palettes.push({
        name: color,
        palette: customPalettes[color] || presetPalette?.[color as KeyOfType<typeof presetPalette>]
    });
});
